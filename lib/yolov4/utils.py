...
from lib.yolov4.layers import FeatureConcat2, FeatureConcat3, FeatureConcat_l, Reorg, JDELayer


def create_modules(module_defs, cfg: DetectConfig):
    # Constructs module list of layer blocks from module configuration in module_defs
    img_size = cfg['net']['imagesize'][0]
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef['type'] == 'convolutional':
            ...
            if bn:
                modules.add_module('BatchNorm2d', torch.nn.BatchNorm2d(filters, momentum=0.03, eps=1E-4))
            else:
                routs.append(i)  # detection output (goes into yolo layer)

            if mdef['activation'] == 'leaky':  # activation study https://github.com/ultralytics/yolov3/issues/441
                modules.add_module('activation', torch.nn.LeakyReLU(0.1, inplace=True))
                ...
                modules.add_module('activation', torch.nn.SiLU())
            ...
            filters = output_filters[-1]
            modules = torch.nn.BatchNorm2d(filters, momentum=0.03, eps=1E-4)
            ...
            if k == 2 and stride == 1:  # yolov3-tiny
                modules.add_module('ZeroPad2d', torch.nn.ZeroPad2d((0, 1, 0, 1)))
            ...
            k = mdef['size']  # kernel size
            stride = mdef['stride']
            avgpool = torch.nn.AvgPool2d(kernel_size=k, stride=stride, padding=(k - 1) // 2)
            if k == 2 and stride == 1:  # yolov3-tiny
                modules.add_module('ZeroPad2d', torch.nn.ZeroPad2d((0, 1, 0, 1)))
            ...
            modules = torch.nn.Upsample(scale_factor=mdef['stride'])
            ...
            if any(x in cfg['base']['file'] for x in ['yolov4-tiny', 'fpn', 'yolov3']):  # P5, P4, P3 strides
                stride = [32, 16, 8]
