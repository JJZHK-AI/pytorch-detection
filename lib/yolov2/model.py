...
def yolov2_darknet19(cfg: DetectConfig):
    return YOLOV2_Darknet19(cfg)


@MODEL_ZOO.register()
def yolov2_resnet50(cfg: DetectConfig):
    return YOLOV2_Resnet50(cfg)


@MODEL_ZOO.register()
def yolov2_darknet19_tiny(cfg: DetectConfig):
    return YOLOV2_Darknet19_Tiny(cfg)
...
class YOLOV2_Net(ModelBase):
    def __init__(self, cfg: DetectConfig):
        ...
        self.input_size = self.cfg['train']['imagesize'][0]
        self.backbone = get_backbone(cfg)
        self.num_anchors = len(self.cfg['base']['anchors'])
        self.anchor_size = torch.tensor(self.cfg['base']['anchors'].astype(np.float32))
...
class YOLOV2_Darknet19(YOLOV2_Net):
    def __init__(self, cfg: DetectConfig):
        super(YOLOV2_Darknet19, self).__init__(cfg)
...
class YOLOV2_Resnet50(YOLOV2_Net):
    def __init__(self, cfg: DetectConfig):
        super(YOLOV2_Resnet50, self).__init__(cfg)
        self.convsets_1 = torch.nn.Sequential(
            Conv(2048, 1024, k=1),
            Conv(1024, 1024, k=3, p=1),
            Conv(1024, 1024, k=3, p=1)
...
class YOLOV2_Darknet19_Tiny(YOLOV2_Net):
    def __init__(self, cfg: DetectConfig):
        super(YOLOV2_Darknet19_Tiny, self).__init__(cfg)
        # detection head
        self.convsets_1 = torch.nn.Sequential(
        ...
        # prediction layer
        self.pred = torch.nn.Conv2d(512, self.num_anchors * (1 + 4 + self.num_classes), kernel_size=1)
