...
import torch
import lib.yolov3.tools as tools
import numpy as np
...
def yolov3_darknet53(cfg: DetectConfig):
    return YOLOV3_Darknet53(cfg)


@MODEL_ZOO.register()
def yolov3_tiny(cfg: DetectConfig):
    return YOLOV3_Tiny(cfg)

@MODEL_ZOO.register()
def yolov3_spp(cfg: DetectConfig):
    return YOLOV3_Spp(cfg)


class YOLOV3_Net(ModelBase):
    def __init__(self, cfg: DetectConfig):
        super(YOLOV3_Net, self).__init__(cfg)
...
class YOLOV3_Darknet53(YOLOV3_Net):
    def __init__(self, cfg: DetectConfig):
        ...
        # s = 32
        self.conv_set_3 = torch.nn.Sequential(
            Conv(1024, 512, k=1),
        ...
        self.pred_3 = torch.nn.Conv2d(1024, self.num_anchors * (1 + 4 + self.num_classes), kernel_size=(1, 1))

        # s = 16
        self.conv_set_2 = torch.nn.Sequential(
            Conv(768, 256, k=1),
        ...
        self.pred_2 = torch.nn.Conv2d(512, self.num_anchors * (1 + 4 + self.num_classes), kernel_size=(1, 1))

        # s = 8
        self.conv_set_1 = torch.nn.Sequential(
            Conv(384, 128, k=1),
        ...
        self.pred_1 = torch.nn.Conv2d(256, self.num_anchors * (1 + 4 + self.num_classes), kernel_size=(1, 1))
    ...
    def __init__(self, in_ch, out_ch, k=1, p=0, s=1, d=1, g=1, act=True):
        ...
        if act:
            self.convs = torch.nn.Sequential(
                torch.nn.Conv2d(in_ch, out_ch, kernel_size=(k, k), stride=(s, s), padding=(p, p),
